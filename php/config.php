...
class BaseDatos_AD{
	...
	public function BaseDatos_AD() {	
		//Incluye el archivo de inicio con los parametros de conexion
		include('db_pac_ini.php');
		//Setea los atributos de la clase con los datos contenidos en el archivo de inicio
		$this->setServidor($hostname);
		$this->setUsuario($username);
		$this->setClave($password);
		$this->setBaseDatos($database);
	}
	...
	public function Conectar(){
		try{
			...
			$conexion = mysqli_connect($this->Str_Servidor,$this->Str_Usuario,$this->Str_Clave,$this->Str_BaseDatos);

			//mysql_connect('127.0.0.1','root','root') or die("no se puede seleccionar la base de datos");
			mysqli_select_db($conexion,$database)
			//mysql_select_db("bdshareboard",$conexion) or die ("No conecto esa base de datos");
			return $conexion;		
		}
